# ERPNext Vue Integration Guide
## Complete Reference for Building Vue.js Frontends on ERPNext Backend

**Last Updated**: 2025-08-11  
**Framework Version**: Vue 3.4+ | ERPNext 15 | Frappe Framework 15  
**Source**: Production integration patterns from CRM and HRMS applications
...
```javascript
// utils/request.js - Custom request handler
import { frappeRequest } from 'frappe-ui'

// Configure the default request handler
export const request = async (options) => {
  try {
    const response = await frappeRequest(options)
    return response
  } catch (error) {
...
export const customersStore = defineStore('customers', () => {
  ...
  // Customer addresses
  const customerAddresses = createResource({
    url: 'frappe.client.get_list',
    makeParams: (customer) => ({
      doctype: 'Address',
  ...
  // Customer contacts
  const customerContacts = createResource({
    url: 'frappe.client.get_list',
    makeParams: (customer) => ({
      doctype: 'Contact',
...
export const salesOrdersStore = defineStore('salesOrders', () => {
  ...
  const salesOrderResource = createResource({
    url: 'frappe.client.get',
    makeParams: (name) => ({
      doctype: 'Sales Order',
      name: name
  ...
  // Sales order items
  const salesOrderItems = createResource({
    url: 'frappe.client.get_list',
  ...
  // Create sales order
  const createSalesOrder = createResource({
    url: 'frappe.client.insert',
    onSuccess(doc) {
  ...
  // Update sales order
  const updateSalesOrder = createResource({
    url: 'frappe.client.save',
    onSuccess() {
      salesOrders.reload()
    }
  ...
  // Submit sales order
  const submitSalesOrder = createResource({
  ...
  // Cancel sales order
  const cancelSalesOrder = createResource({
...
export const userPermissions = createResource({
  url: 'frappe.client.get_list',
  params: {
...
// Employee attendance
export const employeeAttendance = createResource({
  url: 'frappe.client.get_list',
  makeParams: (filters = {}) => ({
    doctype: 'Attendance',
...
// Employee leaves
export const employeeLeaves = createResource({
  url: 'frappe.client.get_list',
  makeParams: (filters = {}) => ({
...
// Check-in resource
export const checkIn = createResource({
...
// Check-out resource
export const checkOut = createResource({
...
export function useQuotationConverter() {
  const convertToSalesOrder = createResource({
